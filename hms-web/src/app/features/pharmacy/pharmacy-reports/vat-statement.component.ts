...
/**
 * "VAT Amount" here is the SGST+CGST sum per day - the app only ever tracked
 * GST (no separate VAT tax type anywhere), so this legacy-named statement
 * reuses the same tax data as GST Statement, grouped by day instead of month.
 */
@Component({
  selector: 'app-vat-statement',
  standalone: true,
  imports: [
    DatePipe,
    DecimalPipe,
  ...
  templateUrl: './vat-statement.component.html',
  styleUrl: './vat-statement.component.scss'
})
export class VatStatementComponent {
  ...
  getDetails(): void {
    ...
    this.service.vatStatement(toIsoDate(this.fromDate()), toIsoDate(this.toDate())).subscribe({
      next: (entries) => {
      ...
      error: () => {
        this.loading.set(false);
        this.notification.error('Failed to load the VAT statement.');
      }
    });
  }
  ...
  print(): void {
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>Pharmacy Vat Statement</title>
